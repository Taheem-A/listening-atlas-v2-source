...
function spotify(env){if(!clients.has(env))clients.set(env,new SpotifyClient(env));return clients.get(env)}
...
// One globally leased batch at a time; the browser supplies only deduplicated IDs.
export async function enrich(ids,cache,client,{clock=Date.now,sleep=ms=>new Promise(r=>setTimeout(r,ms))}={}) {
  ...
  let next=now,records=[],state='ready';
  try{
    ...
    for(const id of ids){
      ...
      try{fetched=await client.track(id)}catch(error){
        const failure=error instanceof SpotifyFailure?error:new SpotifyFailure('temporary_error');
    ...
    return {state,retry_at:next,records};
...
export async function handleAPI(request,env,dependencies={}) {
  ...
  const maximum=path==='/api/spotify/lookup'?1000:8;
  ...
  }catch{return json({state:'temporary_error',records:[],retry_at:Date.now()+60000},503);}
}
